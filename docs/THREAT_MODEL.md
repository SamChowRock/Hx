...
- Notification reads, counts, SSE subscriptions, updates, and deletes derive the recipient only from the Active session and include that user ID in every database predicate. Cross-user notification IDs return `404`; cookie-authenticated mutations retain Origin and CSRF checks. Creation is internal-only, content is bounded plain text, action URLs are safe relative paths, and producers must exclude secrets and unnecessary personal data. Durable business notifications use the transactional outbox and a database-enforced recipient-scoped deduplication key. SSE is an optimization: bounded replay and five-second database reconciliation cover replicas, while authenticated list/count endpoints remain authoritative.
...
- The outbox processor is an intentionally small PostgreSQL poller. BullMQ generalization, queue dashboards, replay tooling, delivery-provider webhooks, notification preference/retention jobs, external push channels, and alerting remain Milestone 3 work.
...
- 通知读取、计数、SSE 订阅、更新与删除只能从 Active Session 得到接收者，并在每条数据库 Predicate 中包含该 User ID。跨用户 Notification ID 返回 `404`；基于 Cookie 的写操作继续执行 Origin 与 CSRF 校验。通知创建只属于内部能力；内容是长度受限的纯文本；Action URL 必须是安全的相对路径；Producer 必须排除 Secret 与不必要的个人数据。可靠业务通知使用事务 Outbox，以及由数据库强制保证、限定在接收者范围内的去重 Key。SSE 只是优化手段：有限 Replay 和每五秒数据库对账负责覆盖多个 Replica，经过认证的列表/计数接口仍是权威结果。
...
- 当前 Outbox Processor 是刻意保持精简的 PostgreSQL Poller。BullMQ 通用化、Queue Dashboard、重放工具、投递 Provider Webhook、通知偏好/保留期 Job、外部推送 Channel 和告警仍属于 Milestone 3。
